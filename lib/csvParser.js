...
// Executes a chart recipe against live rows. Returns a simple or stacked data
// structure ready for ChartRecipeCard to render.
export function computeChartData(rows, recipe) {
  const { metric, xDim, stackDim, chartType, aggregation = "sum" } = recipe;
  if (!metric || !xDim || !rows.length) return null;

  // For line charts: detect raw date xDim and aggregate to month/year periods
  if (chartType === "line" && !stackDim) {
    const sample = rows.slice(0, 30).map((r) => String(r[xDim] ?? "").trim()).filter(Boolean);
    const allDates = sample.length > 0 && sample.every((s) => tryParseDate(s) !== null);
    if (allDates) return _computeDateAggregated(rows, metric, xDim, aggregation);
  }

  return stackDim
    ? _computeStacked(rows, metric, xDim, stackDim, chartType, aggregation)
    : _computeSimple(rows, metric, xDim, aggregation);
}

// Groups a raw date xDim by month or year and aggregates the metric.
function _computeDateAggregated(rows, metric, xDim, aggregation) {
  const dated = rows
    .map((row) => {
      const d = tryParseDate(String(row[xDim] ?? "").trim());
      const val = parseMetricVal(row[metric]);
      return d && val !== null ? { date: d, val } : null;
    })
    .filter(Boolean);
  ...
  dated.forEach(({ date, val }) => {
    const key =
      groupBy === "day"   ? date.toISOString().slice(0, 10) :
      groupBy === "month" ? `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}` :
                            String(date.getFullYear());
    if (!groups[key]) groups[key] = { sum: 0, count: 0 };
    groups[key].sum += val;
    groups[key].count++;
  ...
  const entries = Object.entries(groups)
    ...
    .map(([label, { sum, count }]) => ({
      label,
      fullLabel: label,
      value: aggregation === "avg" ? sum / count : aggregation === "count" ? count : sum,
      count,
    }));

  return { type: "simple", entries };
}

function _computeSimple(rows, metric, xDim, aggregation) {
  const groups = {};
  rows.forEach((row) => {
    const key = String(row[xDim] ?? "").trim();
  ...
  let entries = Object.keys(groups).map((label) => {
    const g = groups[label];
    return {
      label: label.length > 28 ? label.slice(0, 28) + "…" : label,
      fullLabel: label,
      value: aggregation === "avg" ? g.sum / g.count : aggregation === "count" ? g.count : g.sum,
      count: g.count,
      sum: g.sum,
    };
  });

  const ranks = entries.map((e) => getTimeRank(e.fullLabel));
  if (ranks.every((r) => r !== null)) {
    entries.sort((a, b) => (getTimeRank(a.fullLabel) ?? 0) - (getTimeRank(b.fullLabel) ?? 0));
  } else {
...
function _computeStacked(rows, metric, xDim, stackDim, chartType, aggregation) {
  const cells = {};
  const stackTotals = {};

  rows.forEach((row) => {
    const x = String(row[xDim] ?? "").trim();
    const s = String(row[stackDim] ?? "").trim();
    if (!x || !s || isEmpty(x) || isEmpty(s)) return;
    const val = parseMetricVal(row[metric]);
    if (val === null) return;
    if (!cells[x]) cells[x] = {};
    if (!cells[x][s]) cells[x][s] = { sum: 0, count: 0 };
    cells[x][s].sum += val;
    cells[x][s].count++;
    stackTotals[s] = (stackTotals[s] || 0) + val;
  });

  let xLabels = Object.keys(cells);
  const xRanks = xLabels.map((l) => getTimeRank(l));
  if (xRanks.every((r) => r !== null)) {
    xLabels.sort((a, b) => (getTimeRank(a) ?? 0) - (getTimeRank(b) ?? 0));
  }

  const stackLabels = Object.entries(stackTotals)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 8)
    .map(([k]) => k);

  const rawSeries = stackLabels.map((s) => ({
    name: s.length > 22 ? s.slice(0, 22) + "…" : s,
    fullName: s,
    values: xLabels.map((x) => {
      const g = cells[x]?.[s];
      if (!g) return 0;
      return aggregation === "avg" ? g.sum / g.count : g.sum;
    }),
  }));

  const is100 = chartType === "stacked_100";
  let series = rawSeries;
  if (is100) {
    const colTotals = xLabels.map((_, xi) =>
      rawSeries.reduce((acc, ds) => acc + (ds.values[xi] || 0), 0)
    );
    series = rawSeries.map((ds) => ({
      ...ds,
      values: ds.values.map((v, xi) =>
        colTotals[xi] > 0 ? Math.round((v / colTotals[xi]) * 100) : 0
      ),
    }));
  }

  return { type: "stacked", xLabels, series, is100 };
}

// Computes headline KPIs from live rows + recon dataProfile.
export function computeKPIs(rows, dataProfile) {
  if (!dataProfile?.primaryMetric || !rows.length) return null;
  const { primaryMetric, dimensions } = dataProfile;

  const total = rows.reduce((s, row) => {
    const v = parseMetricVal(row[primaryMetric]);
    return v !== null ? s + v : s;
  }, 0);

  // Top entity from "who" dimension
  const whoDim = dimensions?.who?.[0];
  let topEntity = null;
  if (whoDim) {
    const grp = {};
    rows.forEach((row) => {
      const k = String(row[whoDim] ?? "").trim();
      if (!k) return;
      const v = parseMetricVal(row[primaryMetric]);
      if (v !== null) grp[k] = (grp[k] || 0) + v;
    });
    const top = Object.entries(grp).sort((a, b) => b[1] - a[1])[0];
    if (top) topEntity = { name: top[0], value: top[1], dim: whoDim };
  }

  // YoY from year-like "when" column
  const yearDim = dimensions?.when?.find((d) =>
    rows.some((r) => /^\d{4}$/.test(String(r[d] ?? "").trim()))
  );
  let yoy = null;
  if (yearDim) {
    const yGrp = {};
    rows.forEach((row) => {
      const y = String(row[yearDim] ?? "").trim();
      if (!/^\d{4}$/.test(y)) return;
      const v = parseMetricVal(row[primaryMetric]);
      if (v !== null) yGrp[y] = (yGrp[y] || 0) + v;
    });
    const years = Object.keys(yGrp).sort();
    if (years.length >= 2) {
      const curr = yGrp[years[years.length - 1]];
      const prev = yGrp[years[years.length - 2]];
      if (prev > 0) {
        yoy = {
  ...
  return { total, topEntity, yoy, metric: primaryMetric };
}
