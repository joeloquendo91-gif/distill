...
export function generateInsightSentence(col) {
  switch (col.type) {
    case COL_TYPES.NUMERIC: {
      if (!col.stats) return `${col.name} has numeric data across ${col.totalCount.toLocaleString()} responses.`;
      const { mean, min, max } = col.stats;
      return `${col.name} averages ${mean.toLocaleString()}, ranging from ${min.toLocaleString()} to ${max.toLocaleString()}.`;
    }
    case COL_TYPES.LIKERT_NUM: {
      if (col.avg == null) return `${col.name} ratings recorded across ${col.totalCount.toLocaleString()} responses.`;
      const ratio = col.avg / (col.scaleMax || 10);
      const sentiment = ratio >= 0.7 ? "positive" : ratio >= 0.45 ? "moderate" : "low";
      return `Average ${col.name} is ${col.avg} / ${col.scaleMax || 10} — ${sentiment} overall.`;
    }
    case COL_TYPES.LIKERT_TEXT: {
      const top = col.chartData?.[0];
      if (!top) return `${col.name} responses recorded.`;
      return `${top.pct}% of ${col.name} responses were "${top.label}".`;
    }
    case COL_TYPES.CATEGORICAL: {
      const top = col.chartData?.[0];
      if (!top) return `${col.name} has ${col.uniqueCount} unique values.`;
      return `"${top.fullLabel || top.label}" is the most common ${col.name} at ${top.pct}%.`;
    }
    case COL_TYPES.DATE: {
      const peak = col.chartData?.reduce((a, b) => (a.count > b.count ? a : b), { count: 0 });
      if (!peak?.label) return `${col.name} spans multiple time periods.`;
      return `${col.name} peaks in ${peak.label} with ${peak.count.toLocaleString()} entries.`;
    }
    case COL_TYPES.MULTI_SELECT: {
      const top = col.chartData?.[0];
      if (!top) return `${col.name} shows varied selections.`;
      return `"${top.fullLabel || top.label}" is the top ${col.name} choice (${top.count.toLocaleString()} mentions).`;
    }
    case COL_TYPES.TEXT: {
      const topWord = col.chartData?.[0];
      if (!topWord) return `${col.name} contains free-text responses.`;
      return `"${topWord.label}" is the top theme in ${col.name} responses (${topWord.count}×).`;
    }
    default:
      return `${col.name} has data across ${col.totalCount.toLocaleString()} responses.`;
  }
}
...
export function computeTimeSeriesBreakdown(rows, metricCol, dateCol) {
  const dated = rows
    .map((row) => {
      ...
      const raw = String(row[metricCol] ?? "").trim();
      const val = parseFloat(raw.replace(/[$€£¥₹₩,%]/g, "").replace(/,/g, "").trim());
      return !isNaN(d.getTime()) && !isNaN(val) ? { date: d, val } : null;
    })
    .filter(Boolean);
